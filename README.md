# Hi, I'm Kiruthees 👋

🎓 **B.Tech @ IIT Madras**
💻 **Competitive Programmer · Software Engineer**
📈 Interested in **SWE, Quantitative Finance, AI/ML**

I enjoy building things from scratch, solving challenging algorithmic problems, and exploring the intersection of **mathematics, software, and machine learning**.

---

## ⚡ Competitive Programming

* 🟣 **Codeforces:** 1927 — Candidate Master
* 🟡 **CodeChef:** 2057 — 5★
* 🟢 **LeetCode:** Guardian, 1000+ problems solved
* ✍️ **USACO Guide:** Problem solution articles

I particularly enjoy **Dynamic Programming, Graph Algorithms, Data Structures, Optimization, and Problem Solving**.

---

## 🚀 Featured Projects

### 🌐 Network-File-Sync

A network-based file synchronization system exploring **computer networks, concurrency, and distributed communication**.

**C++ · TCP/IP · Multithreading · Networking**

### 🧠 RL-Snake

A **Deep Q-Network (DQN)** agent trained to play Snake using PyTorch, with custom reward shaping and training analysis.

**Python · PyTorch · Reinforcement Learning**

### 💾 Memory-Management-System

A virtual memory simulator implementing **paging, page tables, logical-to-physical address translation, RAM/swap management, and page replacement algorithms**.

**C++ · Operating Systems · LRU · FIFO · Virtual Memory**

---

## 🛠️ Tech Stack

## 🛠️ Tech Stack

### Languages
[![My Skills](https://skillicons.dev/icons?i=cpp,python,java,mysql)](https://skillicons.dev)

### Machine Learning & Data
[![My Skills](https://skillicons.dev/icons?i=pytorch,tensorflow)](https://skillicons.dev)

### Backend & Web
[![My Skills](https://skillicons.dev/icons?i=spring,html,css)](https://skillicons.dev)

### Tools
[![My Skills](https://skillicons.dev/icons?i=git,github,gitlab,vscode,idea)](https://skillicons.dev)

### Other
[![My Skills](https://skillicons.dev/icons?i=latex)](https://skillicons.dev)

---

## 🔬 Currently Exploring

* 🧩 Advanced Algorithms & Competitive Programming
* 📈 Quantitative Finance & Algorithmic Trading
* 🤖 Machine Learning & Reinforcement Learning
* 💻 Operating Systems & Computer Systems

---

## 📫 Connect With Me

[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge\&logo=github\&logoColor=white)](https://github.com/Kiruthees)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge\&logo=linkedin\&logoColor=white)](https://linkedin.com/in/kiruthees-g-b1bab3311/)
[![Codeforces](https://img.shields.io/badge/Codeforces-1F8ACB?style=for-the-badge\&logo=codeforces\&logoColor=white)](https://codeforces.com/profile/Kiruthees5)
[![LeetCode](https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge\&logo=leetcode\&logoColor=black)](https://leetcode.com/u/Kiruthees)
[![CodeChef](https://img.shields.io/badge/CodeChef-5B4638?style=for-the-badge\&logo=codechef\&logoColor=white)](https://www.codechef.com/users/many_petal_95/)

---

### 💡 *Build. Solve. Learn. Repeat.*
